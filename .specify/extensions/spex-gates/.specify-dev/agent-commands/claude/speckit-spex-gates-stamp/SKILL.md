...
metadata:
  ...
  source: spex-gates:commands/speckit.spex-gates.stamp.md
---
...
- If `ask` is `"smart"` or `"never"`: {harness:suppress-prompts-stamp} autonomously, and return immediately so the pipeline can advance.
