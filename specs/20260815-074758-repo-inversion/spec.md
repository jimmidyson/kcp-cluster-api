...
- **SC-005**: The patches carried against upstream are checked automatically without anyone asking — on a schedule, and blocking on any change to the pin or the record — and each is traceable to a proposal filed upstream. Deliberately not on every change: the fork lives in another repository, so gating unrelated work on it produces failures their authors cannot fix.
...
- **Patch set at the start**: Exactly one patch is carried initially — exposing the contract-metadata resolver publicly, which is also the change intended for upstream. That one patch touches **two** paths: the internal file whose lookup becomes overridable, and the new public file exposing the setter. The drift record therefore begins with one entry covering two paths, and the check must compare paths rather than count patches.
