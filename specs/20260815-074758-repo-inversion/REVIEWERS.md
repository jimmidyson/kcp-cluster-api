...
3. **Fork branch cut from a commit, not a release tag.** The fork point
   `281e4e3` is an upstream `main` commit dated after `v1.14.0`. Basing the
   patch branch on the nearest release tag would mean building against a
   different tree than the code was written and tested against. The fork is
   tagged `v1.15.0-kcp.1` — honest about being ahead of v1.14.0 and not yet
   v1.15.0.
