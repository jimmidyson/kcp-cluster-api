...
| Four runs ended their top rung on kube-vip losing its lease nine and a half minutes in, before a fleet that takes ten and three quarters to converge could have arrived. | kube-vip is a static pod, so the death check counted it as the control plane; its exit is the VIP moving, an incident and not the management cluster failing itself. | The four kubeadm components and the managers end a rung; every other death on a control plane node is an incident recorded on the rung, and the ceiling names the clean rung to recommend and the reached rung separately. |
