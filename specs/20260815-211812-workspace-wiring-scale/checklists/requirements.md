...
| I2 | FR-030's "departs from linear" was not a command — no metric, tolerance, point count or fitting rule, so two runs would yield two capacities | FR-030 now specifies the swept quantities, a minimum count of geometrically spaced points, a stated deviation tolerance, and the knee rule (smallest count exceeding the linear projection from the two smallest points by more than tolerance), with tolerance and point count recorded per result |
