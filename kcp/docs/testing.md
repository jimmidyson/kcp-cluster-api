...
All new behavior under `kcp/` is developed test-first:
...
Instead, integration tests for `kcp/` use `kcp/test/integration/envtest`
...
import (
    ...
    "sigs.k8s.io/cluster-api/kcp/test/integration/envtest"
)
...
Integration test files live under `kcp/test/integration/` and carry the
...
`kcp/Makefile`) into `kcp/bin/` and puts it on `PATH` before running the
...
```sh
make -C kcp test   # test-unit + test-integration
```
