...
const clusterCRDRel = "core/config/crd/bases/cluster.x-k8s.io_clusters.yaml"

// resolve returns a manifest path inside a pinned upstream module, failing the
// test with the resolver's own diagnostic if the layout has moved.
func resolve(t *testing.T, module, rel string) string {
	t.Helper()
...
func TestLoadCRD(t *testing.T) {
	clusterCRDPath := resolve(t, ModuleClusterAPI, clusterCRDRel)
...
func TestLoadCRDMachineDeploymentHasDuplicateColumnsBeforeDedupe(t *testing.T) {
	const rel = "core/config/crd/bases/cluster.x-k8s.io_machinedeployments.yaml"
	path := resolve(t, ModuleClusterAPI, rel)
