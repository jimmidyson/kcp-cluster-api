...
// The knee is where capacity comes from, so it has to be a procedure rather
// than a judgement: same points and same tolerance must give the same answer
// to anyone who runs it.

func TestKneeFoundWhereCostDepartsFromLinear(t *testing.T) {
	...
	got := DetectKnee(points, KneeOptions{Tolerance: 0.25})

	if !got.Found {
		t.Fatalf("no knee found in an obviously super-linear series: %s", got.Reason)
	}
	if got.Workspaces != 64 {
		t.Errorf("knee at %d workspaces, want 64", got.Workspaces)
	}
}

func TestNoKneeWhenCostStaysLinear(t *testing.T) {
	points := []Point{
		{Workspaces: 8, Value: 80},
		{Workspaces: 16, Value: 160},
	...
	if got.Found {
		t.Errorf("knee reported at %d for a linear series; a spurious knee understates capacity", got.Workspaces)
	}
	if got.Reason == "" {
		t.Error("no reason recorded for the absent knee: a bare false is not a result somebody can act on")
	}
}

// FR-030's whole point: two runs of one profile must agree.
func TestKneeIsReproducible(t *testing.T) {
	points := []Point{
	...
	opts := KneeOptions{Tolerance: 0.2}

	first := DetectKnee(points, opts)
	for range 10 {
		if again := DetectKnee(points, opts); again != first {
			t.Fatalf("DetectKnee is not deterministic: %+v then %+v", first, again)
		}
	}
}

// A sweep too short to project a trend has not measured anything. Reporting
// "no knee" there would be indistinguishable from "cost is linear", which is
...
func TestTooFewPointsCannotRun(t *testing.T) {
	for _, points := range [][]Point{
		...
		got := DetectKnee(points, KneeOptions{Tolerance: 0.25, MinPoints: 4})
		if got.Found {
			t.Errorf("%d points produced a knee; too short a sweep cannot establish one", len(points))
		}
		if !got.CouldNotRun {
...
// produced it: a knee quoted without them cannot be reproduced or compared.
...
func TestPointsNeedNotBeSuppliedInOrder(t *testing.T) {
	...
	if got, want := DetectKnee(shuffled, KneeOptions{Tolerance: 0.25}), DetectKnee(ordered, KneeOptions{Tolerance: 0.25}); got != want {
		t.Errorf("order changed the answer: %+v vs %+v", got, want)
	}
}

// A tolerance of zero would make measurement noise look like a knee, so it is
// defaulted rather than taken literally.
func TestZeroToleranceIsDefaulted(t *testing.T) {
	points := []Point{
		{Workspaces: 8, Value: 80},
		{Workspaces: 16, Value: 160},
		{Workspaces: 32, Value: 320.5}, // noise, not a knee
		{Workspaces: 64, Value: 641},
	}

	got := DetectKnee(points, KneeOptions{})

	if got.Found {
		t.Errorf("knee at %d from rounding noise; a zero tolerance must not be taken literally", got.Workspaces)
	}
	if got.Tolerance <= 0 {
