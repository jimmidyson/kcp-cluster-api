...
edit. If no such point exists yet, stop and raise it with a human rather
than compromising the invariant.
...
blocker to raise with the user. Options at that point are: find a different
approach (e.g. operate on a wrapped `Manager`), or accept the limitation
until it's solved upstream.
